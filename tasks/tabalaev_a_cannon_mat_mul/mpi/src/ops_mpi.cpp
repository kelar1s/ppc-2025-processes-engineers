...
bool TabalaevACannonMatMulMPI::ValidationImpl() {
  ...
  if (world_rank == 0) {
    ...
    bool basic_validation = (n > 0 && a.size() == n * n && b.size() == n * n);

    bool canon_validation = true;

    if (q * q == world_size) {
      if (n % q != 0) {
        canon_validation = false;
      }
    }
    if (basic_validation && canon_validation) {
      validation = 1;
    }
  }
...
bool TabalaevACannonMatMulMPI::RunImpl() {
  ...
  if (q * q != world_size) {
    if (world_rank == 0) {
      ...
      LocalMatrixMultiply(a, b, global_result, n);
    }
