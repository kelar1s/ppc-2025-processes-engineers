...
- **Распределение данных:** Нулевой процесс загружает входную матрицу и вычисляет параметры для распределения данных:
  ...
  Далее вектор `sendcounts` рассылается всем процессам через `MPI_Bcast`, повзоляя каждому процессу определить размер своего `local_matrix`. После этого матрица распределяется между процессами при помощи функции `MPI_Scatterv`.
...
|------------------|---------------------|------------------------|-----------|---------------|
| **Последовательный** | | | | |
| pipeline | 1 | 0.0112421400 | 1.00x | - |
| task_run | 1 | 0.0113223000 | 1.00x | - |
| **MPI (2 процесса)** | | | | |
| pipeline | 2 | 0.0339955200 | 0.33x | 17% |
| task_run | 2 | 0.0341716400 | 0.33x | 17% |
| **MPI (4 процесса)** | | | | |
| pipeline | 4 | 0.0312923600 | 0.36x | 9% |
| task_run | 4 | 0.0475014400 | 0.24x | 6% |
| **MPI (6 процессов)** | | | | |
| pipeline | 6 | 0.0382975200 | 0.29x | 5% |
| task_run | 6 | 0.0464417800 | 0.24x | 4% |
...
- MPI версии работают в 2.8-4.2 раза медленнее** последовательных.
- Лучший результат: pipeline с 4 процессами (0.36x ускорения).
- Худший результат: task_run с 4 и 6 процессами (0.24x ускорения).
...
2. Параллельная реализация с использованием MPI показала снижение производительности по сравнению с последовательной версией. MPI-версии работают в 2.8-4.2 раза медленнее, что связано с высокими накладными расходами на межпроцессное взаимодействие.
3. При увеличении числа процессов эффективность снижается до 4-5%, что объясняется ростом коммуникационных издержек между процессами при относительно небольшой вычислительной нагрузке задачи поиска минимума.
